...
from editor_app.app import App
from editor_app.text_editor import TextEditor, SYNTAX_RULES
from editor_app.file_explorer import FileExplorer
from editor_app.status_bar import StatusBar
...
class TestTextEditor(unittest.TestCase):
    ...
    def setUp(self):
        self.test_root = tk.Tk()
        # For TextEditor tests, app_instance is primarily for tab text updates.
        # We can mock it without needing a full App instance if App itself isn't under test here.
        self.mock_app_instance = MagicMock() # Simpler mock if full App spec not needed
        self.mock_app_instance.update_tab_text_for_editor = MagicMock()
    ...
    @patch('editor_app.text_editor.re.finditer')
...
class TestFileExplorer(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # Ensure the mock_app has a status_bar attribute, as FileExplorer expects it.
        self.mock_app.status_bar = MagicMock(spec=StatusBar)
        self.file_explorer_frame = tk.Frame(self.test_root)
        # FileExplorer init: master_frame, text_editor_instance (now app_instance for callbacks), app_instance
    ...
    def test_on_file_select_opens_file_in_app(self, mock_isfile):
        self.file_explorer.file_tree.selection = MagicMock(return_value=(
            'I001',))
        self.file_explorer.file_tree.item = MagicMock(return_value=['/fake/path/file.txt', 'file']) # Ensure it returns a list/tuple
    ...
    @patch('editor_app.file_explorer.simpledialog.askstring')
    @patch('os.mkdir')
    ...
    @patch('editor_app.file_explorer.messagebox.askyesno', return_value=True)
    @patch('os.remove')
    @patch('os.path.isfile', return_value=True)
    ...
    @patch('editor_app.file_explorer.FileExplorer._load_icons', MagicMock())
...
class TestApp(unittest.TestCase):
    ...
    def setUp(self):
        self.test_root = tk.Tk()
        # Patch critical UI and initialization methods for App during setup
        with patch('editor_app.app.tk.Tk', MagicMock()) as mock_tk_root, \
             patch('editor_app.app.StatusBar') as MockStatusBar, \
             patch('editor_app.app.FileExplorer') as MockFileExplorer, \
             patch('editor_app.app.TextEditor') as MockTextEditor, \
             patch.object(App, '_create_menu', MagicMock()), \
             patch.object(App, '_setup_search_ui', MagicMock()), \
             patch.object(App, 'update_title_and_status', MagicMock()):

            # Instantiate App - this will use the mocked Tk, StatusBar, FileExplorer etc.
            self.app = App()

        # After instantiation, replace mocks on the instance if needed for specific test interactions
        self.app.status_bar = MockStatusBar.return_value # Use the instance from the mock
        self.app.file_explorer = MockFileExplorer.return_value # Use the instance from the mock
        # self.app.notebook = MagicMock(spec=ttk.Notebook) # mock notebook if not already done by above
        ...
        # Ensure essential attributes are mocked if not covered by class-level patches
        # self.app.status_bar = MagicMock(spec=StatusBar) # Already mocked by patch
    ...
    @patch('os.path.isfile', return_value=True)
    @patch('editor_app.app.TextEditor') # Patching TextEditor where it's looked up by App
    def test_open_file_in_new_tab(self, MockTextEditor, mock_isfile, mock_file_open):
        ...
        mock_editor_instance.is_modified = False # Assume new file is not modified

        # Ensure notebook is mocked on the app instance for this test
        self.app.notebook = MagicMock(spec=ttk.Notebook)
        self.app.notebook.tabs = MagicMock(return_value=[]) # No tabs open initially
        self.app.notebook.add = MagicMock()
        # Mock select to return a consistent, valid new tab ID when a tab is added
        self.app.notebook.select = MagicMock(return_value='.!notebook.!newframe')

        self.app.open_file_in_new_tab('/fake/test.txt')

        self.app.notebook.add.assert_called_once() # A new tab frame should be added
        MockTextEditor.assert_called_once() # TextEditor instance created for the new tab
        mock_editor_instance.set_content.assert_called_once_with('file content', initial_load=True)

        # Check that editor and filepath are stored
        # The key for editors and tab_filepaths will be the return value of notebook.select()
        new_tab_id = self.app.notebook.select.return_value
        self.assertIn(new_tab_id, self.app.editors)
        self.assertEqual(self.app.editors[new_tab_id], mock_editor_instance)
        self.assertIn(new_tab_id, self.app.tab_filepaths)
        self.assertEqual(self.app.tab_filepaths[new_tab_id], '/fake/test.txt')


    def test_open_existing_file_switches_tab(self):
        mock_editor = MagicMock(spec=TextEditor)
        tab_id_1 = '.!notebook.!frame1' # Example tab ID

        # Setup app instance with a mocked notebook and one existing tab
        self.app.notebook = MagicMock(spec=ttk.Notebook)
        self.app.editors = {tab_id_1: mock_editor}
        self.app.tab_filepaths = {tab_id_1: '/fake/existing.txt'}
        self.app.notebook.tabs = MagicMock(return_value=[tab_id_1]) # Simulate one tab open
        self.app.notebook.select = MagicMock() # To check if it's called for switching
        self.app.notebook.add = MagicMock() # To ensure no new tab is added

        self.app.open_file_in_new_tab('/fake/existing.txt')

        self.app.notebook.select.assert_called_with(tab_id_1) # Should select the existing tab
        self.app.notebook.add.assert_not_called() # No new tab should be added
        self.assertEqual(len(self.app.editors), 1) # Editor count should remain 1

    @patch('editor_app.app.messagebox.askyesnocancel', return_value=False) # Target messagebox in app.py
    def test_close_current_tab_no_modifications(self, mock_messagebox_askyesnocancel):
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.is_modified = False
        tab_id = '.!notebook.!frame_to_close'

        # Setup app instance with a mocked notebook
        self.app.notebook = MagicMock(spec=ttk.Notebook)
        self.app.notebook.tabs = MagicMock(return_value=[tab_id]) # Simulate one tab open
        self.app.notebook.select = MagicMock(return_value=tab_id) # This tab is selected
        self.app.notebook.forget = MagicMock() # To check if tab is closed
        ...
        mock_messagebox_askyesnocancel.assert_not_called() # No prompt if not modified
        self.app.notebook.forget.assert_called_once_with(tab_id) # Tab should be closed
        self.assertNotIn(tab_id, self.app.editors) # Editor should be removed from tracking
        self.assertNotIn(tab_id, self.app.tab_filepaths) # Filepath should be removed

    @patch('editor_app.app.messagebox.askyesnocancel', return_value=True) # User clicks "Yes" to save
    @patch.object(App, 'save_file') # Mock the save_file method of App
    def test_close_current_tab_modified_save_yes(self, mock_save_file, mock_messagebox_askyesnocancel):
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.is_modified = True # File is modified
        tab_id = '.!notebook.!frame_modified'

        # Setup notebook mock on the app instance
        self.app.notebook = MagicMock(spec=ttk.Notebook)
        ...
        self.app.editors = {tab_id: mock_editor}
        self.app.tab_filepaths = {tab_id: '/fake/modified_file.txt'}

        # Simulate that save_file successfully "saves" (resets is_modified)
        def side_effect_save_file():
            mock_editor.is_modified = False
        mock_save_file.side_effect = side_effect_save_file

        self.app.close_current_tab()

        mock_messagebox_askyesnocancel.assert_called_once() # Prompt should appear
        mock_save_file.assert_called_once() # save_file should be called
        self.app.notebook.forget.assert_called_once_with(tab_id) # Tab should be closed after save
        self.assertNotIn(tab_id, self.app.editors)
        self.assertNotIn(tab_id, self.app.tab_filepaths)

    def test_on_tab_changed(self):
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.clear_search_highlights = MagicMock() # Method on TextEditor

        # Mock get_current_editor to return our mock_editor
        with patch.object(self.app, 'get_current_editor', return_value=mock_editor), \
             patch.object(self.app, 'update_title_and_status') as mock_update_title_status:

            self.app.search_frame_visible = True # Assume search frame is visible
            self.app.on_tab_changed() # Call the method under test

            mock_update_title_status.assert_called_once() # Title/status should update
            mock_editor.clear_search_highlights.assert_called_once() # Highlights cleared on new tab
            # Verify search context is reset
            self.assertEqual(self.app.last_search_match_info, {'index': "1.0", 'query': ""})


    def test_handle_renamed_file(self):
        tab_id = '.!notebook.!frame_renamed' # Example tab ID
        old_path = '/old/path.txt'
        new_path = '/new/path.txt'
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.is_modified = True # Assume file is modified

        # Setup notebook mock on the app instance
        self.app.notebook = MagicMock(spec=ttk.Notebook)
        self.app.notebook.tabs = MagicMock(return_value=[tab_id])
        self.app.notebook.select = MagicMock(return_value=tab_id) # This tab is active
        self.app.notebook.tab = MagicMock() # To check tab text update

        self.app.tab_filepaths = {tab_id: old_path}
        self.app.editors = {tab_id: mock_editor}

        # Mock update_title_and_status as it might be called if the active tab is renamed
        with patch.object(self.app, 'update_title_and_status') as mock_update_title_status:
            self.app.handle_renamed_file(old_path, new_path)

        self.assertEqual(self.app.tab_filepaths[tab_id], new_path) # Filepath should be updated
        # Tab text should be new name + "*" because is_modified is True
        self.app.notebook.tab.assert_called_once_with(tab_id, text=f"{os.path.basename(new_path)}*")
        mock_update_title_status.assert_called_once() # Title should update if active tab changed


    def test_handle_deleted_file(self):
        tab_id = '.!notebook.!frame_deleted' # Example tab ID
        ...
        self.app.notebook.forget.assert_called_once_with(tab_id) # Tab should be closed
        self.assertNotIn(tab_id, self.app.editors) # Editor removed
        self.assertNotIn(tab_id, self.app.tab_filepaths) # Filepath removed
        mock_update_title_status.assert_called_once() # Title should update


    def test_toggle_search_frame_show_hide(self):
        self.app.search_frame_visible = False
        # Mock editor and its text_area for focus calls
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.text_area = MagicMock(spec=tk.Text) # Mock the Text widget for focus_set
        mock_editor.clear_search_highlights = MagicMock()

        # Mock search_frame and search_entry on the app instance itself
        self.app.search_frame = MagicMock(spec=tk.Frame)
        self.app.search_entry = MagicMock(spec=tk.Entry)


        with patch.object(self.app, 'get_current_editor', return_value=mock_editor):
            # First toggle: Show search frame
            self.app._toggle_search_frame()
            self.app.search_frame.pack.assert_called_once() # Frame should be packed (shown)
            self.assertTrue(self.app.search_frame_visible)
            self.app.search_entry.focus_set.assert_called_once() # Entry should get focus

            # Second toggle: Hide search frame
            self.app._toggle_search_frame()
            self.app.search_frame.pack_forget.assert_called_once() # Frame should be unpacked (hidden)
            self.assertFalse(self.app.search_frame_visible)
            mock_editor.clear_search_highlights.assert_called_once() # Highlights cleared on hide
            mock_editor.text_area.focus_set.assert_called_once() # Editor text area gets focus back


    def test_find_next_found(self):
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.text_area = MagicMock(spec=tk.Text) # Mock Text widget for search, tag_add, see, mark_set
        mock_editor.clear_search_highlights = MagicMock()
        mock_editor.text_area.search.return_value = '1.5' # Simulate found match at index '1.5'

        # Mock search_entry on the app instance
        self.app.search_entry = MagicMock(spec=tk.Entry)
        self.app.search_entry.get.return_value = 'test' # Search query
        self.app.case_sensitive_var = MagicMock(spec=tk.BooleanVar)
        self.app.case_sensitive_var.get.return_value = False # Case-insensitive search

        with patch.object(self.app, 'get_current_editor', return_value=mock_editor):
            self.app._find_next()

        mock_editor.clear_search_highlights.assert_called_once()
        # nocase=True because case_sensitive_var.get() is False
    ...
    def test_find_next_not_found_then_wrap(self):
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.text_area = MagicMock(spec=tk.Text)
        mock_editor.clear_search_highlights = MagicMock()
        # First search returns nothing, second (wrap-around) search finds at '1.2'
        mock_editor.text_area.search.side_effect = ['', '1.2']

        self.app.search_entry = MagicMock(spec=tk.Entry)
        self.app.search_entry.get.return_value = 'wrap'
        self.app.case_sensitive_var = MagicMock(spec=tk.BooleanVar)
        self.app.case_sensitive_var.get.return_value = False # Case-insensitive
        self.app.last_search_match_info = {'index': '5.0', 'query': 'wrap'} # Start search from '5.0'
        ...
        mock_editor.text_area.search.assert_any_call('wrap', '1.0', stopindex='5.0', nocase=True) # Wrap-around call
        mock_editor.text_area.tag_add.assert_called_once_with('search_highlight', '1.2', '1.2+4c')
        # Check status updates for "not found" then "wrapped"
        self.app.status_bar.update_status.assert_any_call("'wrap' not found. Wrapping around.")
        self.app.status_bar.update_status.assert_any_call("Wrapped around. Found: 'wrap'")
    ...
    def test_find_next_case_sensitive_match(self):
        ...
        mock_editor.text_area = MagicMock(spec=tk.Text)
        mock_editor.clear_search_highlights = MagicMock()
        mock_editor.text_area.search.return_value = '1.0' # Found at '1.0'

        self.app.search_entry = MagicMock(spec=tk.Entry)
        self.app.search_entry.get.return_value = 'Test'
        self.app.case_sensitive_var = MagicMock(spec=tk.BooleanVar)
        self.app.case_sensitive_var.get.return_value = True # Case-sensitive search

        with patch.object(self.app, 'get_current_editor', return_value=mock_editor):
            self.app._find_next()

        args, kwargs = mock_editor.text_area.search.call_args
        self.assertFalse(kwargs.get('nocase', True), 'Search should be case-sensitive (nocase=False)')
    ...
    def test_find_next_case_insensitive_match_via_option(self):
        ...
        self.app.search_entry.get.return_value = 'test'
        self.app.case_sensitive_var = MagicMock(spec=tk.BooleanVar)
        self.app.case_sensitive_var.get.return_value = False # Case-insensitive search explicitly set

        with patch.object(self.app, 'get_current_editor', return_value=mock_editor):
            self.app._find_next()

        args, kwargs = mock_editor.text_area.search.call_args
        self.assertTrue(kwargs.get('nocase', False), 'Search should be case-insensitive (nocase=True)')
        self.app.status_bar.update_status.assert_called_with("Found: 'test'")
    ...
    def test_find_previous_case_sensitive_match(self):
        mock_editor = MagicMock(spec=TextEditor)
        mock_editor.text_area = MagicMock(spec=tk.Text)
        mock_editor.text_area.index = MagicMock(return_value="3.0") # Mock current cursor position
        mock_editor.clear_search_highlights = MagicMock()
        mock_editor.text_area.search.return_value = '1.0' # Found at '1.0'
        ...
        args, kwargs = mock_editor.text_area.search.call_args
        self.assertFalse(kwargs.get('nocase', True), 'Search should be case-sensitive (nocase=False)')
        self.assertTrue(kwargs.get('backwards'), 'Search should be backwards')
        self.app.status_bar.update_status.assert_called_with("Found: 'Test'")
    ...
    def test_on_search_option_changed_clears_last_match(self):
        ...
        self.app.search_entry = MagicMock(spec=tk.Entry) # Mock search_entry
